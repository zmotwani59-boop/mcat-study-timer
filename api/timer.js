...
async function getRecentEntries(base, workspaceId, userId, headers) {
  ...
  const activeEntries = entries
    .filter((item) => item?.timeInterval?.start && !item?.timeInterval?.end)
    .sort(
      (a, b) =>
  ...
  const intervals = entries
    .map((entry) => {
      ...
      const end = entry?.timeInterval?.end
        ? Date.parse(entry.timeInterval.end)
        : now.getTime();
...
function cleanDescription(value) {
  ...
  if (typeof parsed === "object") {
    ...
    if (typeof parsed?.plain_text === "string") {
      return parsed.plain_text.trim() || null;
    }
  }
  ...
  try {
    ...
    for (const region of regions) {
      ...
      if (firstReadable) {
        return response.status(200).json(
          successPayload({
            entries: firstReadable.result.entries,
            activeEntry: null,
            region,
            userId,
            workspace: firstReadable.workspace,
            timeZone,
          })
        );
      }
